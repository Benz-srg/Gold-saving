...
const HistoryPage = ({ user }) => {
  ...
  const loadhisdata = async () => {
    ...
    register.forEach((element) => {
      ...
      var detail = "";
      if (element.status === "nopass") {
        detail = "อนุมัติการสมัคร";
      }
      var row = {
        name: element.users[0].fname + " " + element.users[0].lname,
        detail: detail,
        for: element.fname + " " + element.lname,
        date: moment(element.dateCreate).format("YYYY-MM-DD HH:mm:ss"),
      };
      data.push(row);
    });
    var columns = [
  ...
  return (
    <>
      ...
      <div className="card mt-5">
        <div className="card-header">ประวัติการอนุมัติข้อมูลผู้ใช้</div>
